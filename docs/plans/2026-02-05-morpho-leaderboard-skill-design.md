...
2. Query `vaultV2s` per chain (pagination with `first` and `skip`), fetching vault metadata, asset address, total assets, total assets USD, net APY, whitelisted status, and warnings. If a 5xx occurs, reduce page size and retry from the start for that chain.
...
- Retry 5xx once; if persistent, auto-downgrade `first` or throttle requests.
